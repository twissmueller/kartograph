---
name: angular-kotlin
title: Angular front end with a Kotlin back end
status: scaffold
version: 0
targets: [web, server-jvm]
docs: [design-system.md, code-design.md, build-design.md]
---

# Angular front end with a Kotlin back end

TypeScript with Angular in the browser, Kotlin (Ktor or Spring) on the server.
...
- an `angular.json` at the root or under a front-end directory;
- a Kotlin server build (`settings.gradle.kts` with a JVM application module) beside it.
...
run-local.sh (docker, down, logs, status), prepare-release.sh, deploy.sh (backend to Fly, frontend to Vercel). Their contract is `stacks/common/DISTRIBUTION.md`.
