...
run-local.sh (desktop, ios, android, docker), run-device.sh, prepare-release.sh, deploy-testflight.sh, deploy-play-internal.sh, push-store-metadata.sh, release-check.sh, first-release-check.sh, release-stores.sh. Their contract is `stacks/common/DISTRIBUTION.md`.
