...
| **Walk** | Acceptance | `/karto-walk [scope]` | **Drives your running app** — via the Claude in Chrome extension, Playwright, or a Compose Multiplatform app's Compose Hot Reload MCP server — showing you each **Developed** scenario's feature and full text first, then performing it in front of you, one at a time, in plain language. It both shows you what was built and checks it works, then asks after every scenario whether it was implemented correctly. Pass marks it **Accepted**; Fail records why (a scenario note) and points you at `/karto-build`. The only way to reach Accepted. |
