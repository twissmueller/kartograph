...
The default design system for every Kotlin Multiplatform app. One token set, two modes
...
- **One entry point:** `AppTheme { content }` in the `shared` module at
  `shared/src/commonMain/kotlin/<package>/ui/theme/`. It wraps `MaterialTheme` and
