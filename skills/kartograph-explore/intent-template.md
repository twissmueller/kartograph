---
title: <short name for what the person wants>
date: <YYYY-MM-DD>
status: draft | confirmed
role: <the role the person spoke from>
language: <language of the conversation>
sources: <issue, ticket, URL, or document the request came from — or none>
related: <earlier intents/… files this one revisits or builds on — or none>
---

# <title>

## Summary

<One paragraph. What the person wants, why, and for whom. A reader with two minutes should
stop here and know the intent.>

## Who

- **Speaking:** <the person's role and what that role is responsible for here>
- **Benefits:** <who gains when this is done>
- **Affected:** <who else has to change habits, tools, or expectations>

## Goals

<Why the work exists. One bullet per goal, in the person's words where possible.>

## Intended outcomes

<What will observably be true when it is done. How the person said they would recognise
success. One bullet each, concrete enough that someone could check it.>

## Non-goals

<What is deliberately left out, each with the reason it was left out.>

## Constraints

<What cannot change: time, money, platform, law, existing systems, people.>

## Assumptions

<What either side is taking for granted. Each one stated so it can be denied later.>

## Decisions

<One entry per choice made in the conversation.>

- **<decision>** — because <reason>. Rejected: <alternative(s) and why>.

## Open questions

<One entry per question the person could not answer yet.>

- **<question>** — who can answer: <person or role>. Why it matters: <what changes depending
  on the answer>.

## Terms

<Words the person used with a specific meaning, each with the meaning as used in this
conversation. Note where two words were used for the same thing and which one was chosen.>

## Notes

<Anything said that fits no section above and should not be lost.>
